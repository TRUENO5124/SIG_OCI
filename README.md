...
STM32F103C8T6 本身没有片上 DAC，因此本项目使用 `PA6` 的 PWM 输出模拟信号发生器。方波可直接由 PWM 输出；正弦波、三角波和锯齿波通过周期性调整 PWM 占空比实现。由于课程要求不增加外部电路，项目未加入额外低通滤波电路。
...
| 测试项目 | 测试方法 | 当前结果 | 备注 |
| --- | --- | --- | --- |
| HTML 面板启动 | 双击 `run_instrument_panel.bat` | 待补充 | 后续可补充软件启动截图 |
| 模拟器连接 | 点击 `连接模拟器`，连接 `127.0.0.1:7897` | 待补充 | 后续可补充连接成功截图 |
| 信号发生器参数设置 | 在面板中设置波形、频率、幅度、偏置 | 待补充 | 后续可补充不同波形截图 |
| 示波器采样显示 | 点击 `运行` 或 `单次` 显示采样波形 | 待补充 | 后续可补充波形显示截图 |
| 真实开发板串口连接 | 点击 `连接 DAPmini 串口` 并选择 COM 口 | 待补充 | 后续可补充真实硬件测试结果 |
| PA6 到 PA0 闭环测试 | 使用杜邦线连接 `PA6 -> PA0` | 待补充 | 后续可补充实测波形 |
...
截图文件可后续放入 `docs/images/` 目录，并在下方替换占位内容。
...
<!-- 示例：
![HTML 仪器面板主界面](docs/images/panel-main.png)
-->
...
<!-- 示例：
![连接模拟器成功界面](docs/images/panel-simulator-connected.png)
-->
...
<!-- 示例：
![信号发生器设置界面](docs/images/generator-settings.png)
-->
...
<!-- 示例：
![示波器波形显示界面](docs/images/scope-waveform.png)
-->
...
<!-- 示例：
![真实硬件连接与测试截图](docs/images/hardware-test.png)
-->
